...
exports.create = (req, res) => {
  ...
  if (!req.body.idcategory) {
    return res.status(400).send({
      message: "El id de categoria no puede ser vacío",
    });
  }
  ...
  if (!req.body.aroma) {
    return res.status(400).send({
      message: "La aroma no puede ser vacía",
    });
  }
  if (!req.body.appearance) {
    return res.status(400).send({
      message: "La apariencia no puede ser vacía",
    });
  }
  if (!req.body.flavor) {
    return res.status(400).send({
      message: "El sabor no puede ser vacía",
    });
  }
  if (!req.body.mouthfeel) {
    return res.status(400).send({
      message: "La sensacion no puede ser vacía",
    });
  }
  if (!req.body.impression) {
    return res.status(400).send({
      message: "La impresion no puede ser vacía",
    });
  }
  if (!req.body.comments) {
    return res.status(400).send({
      message: "Los comentarios no pueden ser vacíos",
    });
  }
  if (!req.body.history) {
    return res.status(400).send({
      message: "La historia no puede ser vacía",
    });
  }
  if (!req.body.ingredients) {
    return res.status(400).send({
      message: "Los ingredientes no pueden ser vacíos",
    });
  }
  if (!req.body.comparison) {
    return res.status(400).send({
      message: "La comparasion no puede ser vacía",
    });
  }
  if (!req.body.examples) {
    return res.status(400).send({
      message: "Los ejemplos no pueden ser vacíos",
    });
  }
  if (!req.body.tags) {
    return res.status(400).send({
      message: "Los tags no pueden ser vacíos",
    });
  }
...
exports.update = (req, res) => {
  ...
  if (!req.body.name) {
    return res.status(400).send({
      message: "El nombre de sub categoria no puede ser vacío",
    });
  }
 ...
 exports.delete = (req, res) => {
  Subcategory.findByIdAndRemove(req.params._id)
    .then((subcategories) => {
      ...
      res.send({ message: "La sub categoria se eliminó de manera correcta!" });
    })
    .catch((err) => {
